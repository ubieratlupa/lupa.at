...
class ImageResizeJob < ApplicationJob
  ...
  def perform(photo_id)
    ...
    stdout, stderr, status = Open3.capture3("curl -L #{Shellwords.escape(download_url)} | gm convert -size 2000x2000 - -resize '2000x2000>' +profile '*' /var/www/upload/image/#{Shellwords.escape(filename)}")
    ...
    unless status.success?
      ...
      raise "Image Resize Command failed"
    end
